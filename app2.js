...
document.querySelector('#btnGoToTwo').addEventListener('click', () => {
    ...
    if (firstName.value.trim() === "") {
        errorAlert.textContent = "VALIDATION ERROR: First Name required, cannot be empty.";
        errorAlert.classList.remove('d-none');
        return; 
    }

    else if (lastName.value.trim() === "") {
        errorAlert.textContent = "VALIDATION ERROR: Last Name required, cannot be empty.";
        errorAlert.classList.remove('d-none');
        return; 
    }

    else if (userAge.value.trim() === "") {
        errorAlert.textContent = "VALIDATION ERROR: Age required, cannot be empty.";
        errorAlert.classList.remove('d-none');
        return; 
    }
    ...
    if (userBirthday.value.trim() === "") {
        errorAlert.textContent = "VALIDATION ERROR: Birthday required, cannot be empty.";
        errorAlert.classList.remove('d-none');
        return; 
    }
...
document.querySelector('#btnGoToThree').addEventListener('click', () => {
    ...
    if (currentAddress.value.trim() === "" || permanentAddress.value.trim() === "") {
        errorAlert.textContent = " VALIDATION ERROR: Address require right location.";
        errorAlert.classList.remove('d-none');
        return;
    }
